    ...
    in
    {
      ...
      packages = forAll (
        ...
        rec {
          ...
          plugin =
            pkgs.runCommand "nixarchy-flatsnap-plugin"
              {
                meta = with pkgs.lib; {
      ...
      checks = forAll (
        ...
        {
          shellcheck =
            pkgs.runCommand "nixarchy-flatsnap-shellcheck" { nativeBuildInputs = [ pkgs.shellcheck ]; }
              ''
          ...
          cli =
            pkgs.runCommand "nixarchy-flatsnap-cli"
              {
          ...
          manifest = pkgs.runCommand "nixarchy-flatsnap-manifest" { nativeBuildInputs = [ pkgs.jq ]; } ''
          ...
          module-gating =
            ...
            assert lib.assertMsg (snapdWith {
              snaps = [ { name = "hello-world"; } ];
            }) "snapd off with a snap declared";
            assert lib.assertMsg (snapdWith {
              pendingRemoval = [ "hello-world" ];
            }) "snapd off while a removal is pending";
              ...
              (nixpkgs.lib.nixosSystem {
                ...
                modules = [
                  ...
                  self.nixosModules.flatsnap
                  {
                    boot.isContainer = true;
                    system.stateVersion = "26.05";
                    programs.nixarchy.flatsnap.snaps = [ { name = "hello-world"; } ];
                  }
                ];
              }).config.systemd.services.nixarchy-flatsnap-snaps.path != [ ]
            ) "flatsnap output does not compose with an existing nix-snapd import";
              ...
              !(
                (nixpkgs.lib.nixosSystem {
                  ...
                  modules = [
                    ...
                    nix-flatpak.nixosModules.nix-flatpak
                    {
                      options.programs.nixarchy.menu.extraEntries = lib.mkOption {
                ...
                }).config.programs.nixarchy.menu.extraEntries
                  ? "install.flatsnap"
              )
            ) "the module still adds its own menu row";
          ...
          module = pkgs.testers.runNixOSTest {
                ...
                flatpaks = [
                  {
                    ...
                    overrides.Context.filesystems = [ "xdg-pictures:ro" ];
                  }
                ];
                snaps = [ { name = "hello-world"; } ];
              };
            ...
            testScript =
              ...
              in
              assert lib.assertMsg (
                lib.elem "com.valvesoftware.Steam" ids && lib.elem "org.gnome.Calculator" ids
              ...
              assert lib.assertMsg (
                cfg.overrides."org.gnome.Calculator".Context.filesystems == [ "xdg-pictures:ro" ]
              ) "override lost";
              ''
